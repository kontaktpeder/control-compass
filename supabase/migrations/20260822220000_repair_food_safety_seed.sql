-- Re-seed Food Safety when the playbook row exists but obligations were never created.
...
BEGIN
  FOR r IN
    SELECT id FROM public.organizations WHERE kind IS DISTINCT FROM 'holding'
  LOOP
