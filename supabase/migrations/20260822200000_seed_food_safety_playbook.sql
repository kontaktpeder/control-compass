-- Food Safety playbook for operating (and sole prop) food businesses.
-- Holding companies are not seeded. Idempotent on playbooks.slug = 'food_safety'.
...
REVOKE EXECUTE ON FUNCTION public.seed_food_safety_playbook(uuid) FROM public, anon;
GRANT EXECUTE ON FUNCTION public.seed_food_safety_playbook(uuid) TO authenticated;

-- Existing operating / sole-prop orgs get the playbook; holding is skipped.
DO $$
DECLARE r record;
BEGIN
  FOR r IN
    SELECT id FROM public.organizations WHERE kind IN ('operating', 'sole_prop')
  LOOP
    PERFORM public.seed_food_safety_playbook(r.id);
  END LOOP;
END $$;
